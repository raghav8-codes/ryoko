...
const BusList = ({ filters }) => {
  ...
  const handleAdd = (bus) => {
    setCart((prev) => {
      const currentQty = prev[bus.id]?.quantity || 0;
      if (currentQty < bus.seatsAvailable) {
        return {
          ...prev,
          [bus.id]: {
            ...bus,
            quantity: currentQty + 1,
            subtotal: (currentQty + 1) * bus.fare,
          },
        };
      }
      return prev;
    ...
    <section className="bus-list">
      ...
      {filteredBuses.length === 0 ? (
        ...
        filteredBuses.map((bus) => {
          const inCart = cart[bus.id]?.quantity || 0;
          ...
          return (
            <div className="bus-card" key={bus.id}>
              ...
              <p><strong>Seats Left:</strong> {bus.seatsAvailable - inCart}</p>
              <p><strong>Fare:</strong> ₹{bus.fare}</p>

              <div className="quantity-controls">
                <button onClick={() => handleRemove(bus)} disabled={inCart === 0}>−</button>
                <span>{inCart}</span>
                <button onClick={() => handleAdd(bus)} disabled={inCart >= bus.seatsAvailable}>+</button>
              </div>
            </div>
          );
        })
